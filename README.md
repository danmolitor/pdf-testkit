...
filter with `minConfidence`.

| Capability | FormePDF `LayoutInfo` | Raw PDF (pdfjs) |
| --- | :-: | :-: |
| Page count / page-assignment | ✅ reliable | ✅ reliable |
| Element added / removed / moved | ✅ reliable | ✅ reliable |
| Heading hierarchy | ✅ reliable | ✅ decent (font-size clustering) |
| Table position | ✅ reliable | ⚠️ best-effort (borderless heuristic) |
| Text overflow | ✅ reliable | ⚠️ best-effort (inferred margin box) |

Known heuristic limits on the pdfjs path: ruled/border-only tables (no text geometry) and
merged/wrapped cells are not reliably detected; overflow is approximate because a raw PDF has no
container concept. Position these as FormePDF-first features.
